...
#[derive(Parser)]
#[command(name = "bhcli")]
#[command(author = "Dantca <Dantca@protonmail.com>")]
...
impl LeChatPHPClient {
    ...
    fn start_keepalive_thread(
        ...
        let msg_keep = ">>> H-E-L-L-O C-H-A-T-T-E-R-S W-E-L-C-O-M-E B-A-C-K TO BHC <<<  
                            Dont Break The !-rules, and stay calm, 
...
fn n0tr1v_only_process_msg(from: &str, msg: &str, tx: &crossbeam_channel::Sender<PostType>, sig: &Arc<Mutex<Sig>>, &user) {
    // !bhcli filters
    if msg == "!bhcli" {
    ...
    if from != N0TR1V && from != FAGGOT {
        if msg.contains(FAGGOT) && (msg.contains("pedo")
         || msg.contains("child")) {
            let msg = "dont break the rules !! you can try !-help or !-rules before".to_owned();
    ...
    if BAN_IMPOSTERS {
        if users.admin.len() == 0 && (users.staff.len() == 0 || account_username == N0TR1V) {
            ...
            for (_color, username) in &users.guests {
                ...
                if n0tr1v_rgx.is_match(&lower_name) 
                || lower_name.contains("pedo")
                ||lower_name.contains("cp")
                ||lower_name.contains("danbyt")
                ||lower_name.contains("bigdick")
                ||lower_name.contains("bitch")
                ||lower_name.contains("kill")
                ||lower_name.contains("killer")
                ||lower_name.contains("dick")
                ||lower_name.contains("trolls")
                ||lower_name.contains("child porn")
                ||lower_name.contains("hamas")
                ||lower_name.contains("pussy")
                ||lower_name.contains("cum")
                ||lower_name.contains("pedofile")
                ||lower_name.contains("fucked")
                ||lower_name.contains("lolita slaves")
                ||lower_name.contains("fuck all")
                ||lower_name.contains("fucking")
                ||lower_name.contains("bomb")
                ||lower_name.contains("fuckings")
                 {
                    let msg = "do not use names on the blacklist ( ~xpldan bot )".to_owned();
