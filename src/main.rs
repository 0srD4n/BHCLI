...
const USERNOPE: [&str;7] = ["xpldan", "maple", "aiden", "sabme", "stuxnet", "rex", "xpltest"];
...
impl LeChatPHPClient {
    ...
    //                 tx.send(PostType::Post(msg_keep.to_owned(), Some(SEND_TO_ALL.to_owned()))).unwrap();
                thread::spawn(move || loop {
                    let dantcahelp = || {
                        let help_message = "[color=#ffffff]list of commands for Guests to dantca !![/color]
                        [color=#ffffff]reportdan-![/color] = to send report bug or something wired or if you have suggestions and input you can do this.
                        [color=#ffffff]chatlogs-![/color] = to see chat logs.
                        [color=#ffffff]danbot-![/color] = to see dantca help.
                        [color=#ffffff]bhcli-![/color]  = for display terminal interfaces bhc
                        [color=#FF0000]without [bold]( - )[/bold][/color]
                        [color=#ffffff]note: the bot can gready. and still beta. so dont be anggry have fun to cyber space : )[/color]";
                        tx.send(PostType::Post(help_message.to_owned(), Some(SEND_TO_ALL.to_owned()))).unwrap();
                    };
                    let keep_msg = || {
                    ...
                    let clb = || {
                        tx.send(PostType::Post("<keepalive>".to_owned(), Some("0".to_owned()))).unwrap();
                        tx.send(PostType::DeleteLast).unwrap();
                    };
                    let timeout = after(Duration::from_secs(60 * 75));
                    let help = after(Duration::from_secs(500));
                    select! {
                        recv(&last_post_rx) -> _ => {},
                        recv(&exit_rx) -> _ => return,
                        recv(&timeout) -> _ => { clb(); keep_msg(); },
                        recv(&help) -> _ => { dantcahelp(); }
                    }
                })
    }
    ...
    should_notify: &mut bool) -> anyhow::Result<()>
{
    ...
    };
    {
        ...
        // Bangun vektor pesan. Tandai pesan yang dihapus.
    ...
    {
        let mut userss = users.lock().unwrap();
        ban_imposters(&tx, &username, &userss);
        *userss = extract_users(&doc);
    }
    Ok(())
}
...
fn process_new_messages(
    new_messages: &[Message], messages: &MutexGuard<Vec<Message>>,
    datetime_fmt: &str, members_tag: &str, username: &str, should_notify: &mut bool,
    tx: &crossbeam_channel::Sender<PostType>,
    users: &Arc<Mutex<Users>>,
) {
    let mut chatlog_file = OpenOptions::new()
        .write(true)
        .append(true)
        .create(true)
        .open("/home/xpldan/Personal/bhcli/chatlog/chatlog.txt")
        .expect("Tidak dapat membuka file chatlog.txt");
        ...
        if let Some(last_known_msg) = messages.first() {
            ...
            for new_msg in filtered {
                if let Some((from, to_opt, msg)) = get_message(&new_msg.text, members_tag) {
                    unsafe { if BOT_ACTIVE {
                        if from != "XplDan"{
                    dantca_absen(&msg, &from, &tx);
                        }
                    } }
                    // Tulis pesan baru ke file chatlog
                    writeln!(chatlog_file,"{} - {}: {}", new_msg.date, from, msg).expect("Gagal menulis ke file chatlog");
                   ...
                   if from == XPLDAN || users_lock.members.iter().any(|(_color, username)| username.to_lowercase() == from.to_lowercase()) {
                    let fromsa = from.to_lowercase();
                       if msg.contains("dantcaoff!") {
                        ...
                        if msg.contains("reportdan!") {
                            reports_msg_bot(&tx, &from);
                        }
                        if fromsa !="xpldan" {
                        if msg.contains("danbot!") {
                            help_command(&tx, &from, &users_lock, &msg);
                        }}
                        if msg.contains("chatlogs!") {
                            chatlog_fun(&tx, &from);
                        }
                        if msg.contains("regbotdan!"){
                            register_user(&tx, &from);
                        }
                    } else {
                        if msg.contains("danbot!") {
                            help_command(&tx, &from, &users_lock, &msg);
...
fn help_command(tx: &crossbeam_channel::Sender<PostType>, from: &str,users:&Users, msg : &str) {
    if users.members.iter().any(|(_color, username)| username.to_lowercase().contains(&from)) {
        if  msg.contains("danbot!") {
            if from != "xpldan" {
            tx.send(PostType::Post(format!("Hallo @{} 
            [color=#ffffff]list of commands for Members to dantca !![/color]
            [color=#ffffff]dantcago-![/color] = to turn on dantca.
            [color=#ffffff]dantcaoff-![/color] = to deactived dantca.
            [color=#ffffff]statusdan-![/color] = to see whether dantca status is activated or deactivated.
            [color=#ffffff]reportdan-![/color] = to send report bug or something wired or if you have suggestions and input you can do this.
            [color=#ffffff]regbotdan-![/color] = If you are not on the list then you can use this command.
            [color=#ffffff]chatlogs-![/color] = to see chat logs.
            [color=#ffffff]danbot-![/color] = to see dantca help.
            [color=#FF0000]without ( - )[/color]
            [color=#ffffff]note: the bot can gready. and still beta. so dont be anggry have fun to cyber space : )[/color] ", from), Some(SEND_TO_MEMBERS.to_owned()))).unwrap();
            }
        }
    } else if  !users.members.iter().any(|(_color, username)| username.to_lowercase().contains(&from)) {
        tx.send(PostType::Post(format!("Hallo @{} 
        [color=#ffffff]list of commands for Guests to dantca !![/color]
        [color=#ffffff]reportdan-![/color] = to send report bug or something wired or if you have suggestions and input you can do this.
        [color=#ffffff]chatlogs-![/color] = to see chat logs.
        [color=#ffffff]danbot-![/color] = to see dantca help.
        [color=#ffffff]bhcli-![/color]  = for display terminal interfaces bhc
        [color=#FF0000]without ( - )[/color]
        [color=#ffffff]note: the bot can gready. and still beta. so dont be anggry have fun to cyber space : )[/color] ", from), Some(SEND_TO_ALL.to_owned()))).unwrap();
    }  
}
/////!function dantca
fn chatlog_fun(tx: &crossbeam_channel::Sender<PostType>, from: &str){
    if from != "xpldan" {
    let chatlog_path = "/home/dantca/Personal/bhcli/chatlog/chatlog.txt";
    let pesan_pm = format!("Hello @{}, the chatlog file will be sent via PM, please be patient.. -<", from);
    tx.send(PostType::Post(pesan_pm, Some(from.to_owned()))).unwrap();
    tx.send(PostType::Upload(chatlog_path.to_owned(), from.to_owned(), "".to_owned())).unwrap();
    }
}
fn reports_msg_bot(tx: &crossbeam_channel::Sender<PostType>, from: &str ){
    let pesan_pm = format!("Hallo , @{} You can send a message here. To send a message to XplDan, click the '-All chatters-' box above the chat, and select user @XplDan",from);
    let username_tujuan = from.to_owned();
    tx.send(PostType::Post(pesan_pm, Some(username_tujuan))).unwrap(); 
}
fn toggle_bot_active(active: bool, tx: &crossbeam_channel::Sender<PostType>, from: &str) {
    ...
    let message = format!("{}", status_message);
    tx.send(PostType::Post(message, None)).unwrap();
}
...
fn check_bot_status(tx: &crossbeam_channel::Sender<PostType>, from: &str) {
    ...
    tx.send(PostType::Post(messtats.to_string(), None)).unwrap();
}

fn register_user(tx: &crossbeam_channel::Sender<PostType>, from: &str) {
    if USERNOPE.contains(&from) {
    let messagea = format!("Hallo, @{}, Your account has been registered in the dantca family group --0<",from);
    let username_tujuan = from.to_owned();
    tx.send(PostType::Post(messagea, Some(username_tujuan))).unwrap();  
  } else {
        let message = format!("Hallo @{}, -> registering an account begins... -> please wait... ->",from);
        let username_tujuan = from.to_owned();
        tx.send(PostType::Post(message, Some(username_tujuan))).unwrap();
        }
}
// dantca no here 

fn dantca_absen(msg : &str,from : &str, tx : &crossbeam_channel::Sender<PostType>){
    if msg.contains("[M]") {return}
    if msg.contains("xpldan") || msg.contains(format!("@{}",XPLDAN).as_str()) {
        let msgafk = format!("Hello @{}, @XplDan [color=#f9f099]is not available at the moment, he changed his mode to Dantca. We will forward your message to xpldan when he returns![/color]",from);
        tx.send(PostType::Post(msgafk,Some(SEND_TO_ALL.to_owned()))).unwrap();
    }
}

// fn dantca_afk(msg: &str, from : &str, tx: &crossbeam_channel::Sender<PostType>){
//   let  msgcopy = msg.to_lowercase();
//     if msg.contains("[M]") {return}
//     if msgcopy.contains("xpldan")|| msgcopy.contains("@xpldan"){
//         if from != "Xpldan" || from != "XplDan"  {
//             let msg_afk = format!("Hello @{}, @XplDan is not available at the moment, he changed his mode to Dantca BOT. We will forward your message to xpldan when he returns!",from);
//             tx.send(PostType::Post(msg_afk,None)).unwrap();
//         }
//     }
// }
// 
// !bot by dantca XplDan
fn dantca_imps_proses(from: &str, msg: &str, tx: &crossbeam_channel::Sender<PostType>, users: &Users) {
    // let fromsa = from.to_lowercase();
    // if fromsa != "xpldan" || fromsa != "@xpldan"  {
    //     dantca_afk(&msg, &from, &tx);
    // }
    ...
    for (_color, username) in &users.guests {
        if from == username.to_lowercase().as_str() {
            ...
            // Initialize or update the warning count for the user
            let mut warned_users = WARNED_USERS.lock().unwrap();
            let count = warned_users.entry(from.to_owned()).or_insert(0);
            if triggered {
                *count += 1;
                tx.send(PostType::Post(format!("-> Hallo @{} -> your warns: [color=#ffffff]you have warns[/color] [color=#00FF00]| {}/2 |[/color] -> [color=#ffffff]{}[/color] [color=#f734ee][BANNED TOPIC][/color]-< ", username_to_kick, *count, warns), Some(SEND_TO_ALL.to_owned()))).unwrap();
            }
            if *count >= 2 { // Jika pengguna telah mendapat peringatan 2 kali
    ...
    if BAN_IMPOSTERS {
        ...
        if users.admin.len() == 0 && (users.staff.len() == 0 || account_username == XPLDAN) {
            ...
            for (_color, username) in &users.guests {
                ...
                if n0tr1v_rgx.is_match(&lower_name) || banned_keywords.iter().any(|&word| lower_name.contains(word)) {
                    let msg = format!("Do not use names on the blacklist '{}'.", lower_name);
                    tx.send(PostType::Kick(msg, username.to_owned())).unwrap();
                }
                if account_username == XPLDAN {
                    if lower_name.contains("fuck")
                    ||lower_name.contains("xpldan")
                    
                        || lower_name.contains("niggi")
                        || lower_name.contains("niggu")
                        || lower_name.contains("nigge")
                        || lower_name.contains("nigga")
                        || lower_name.contains("niggo")
        
                        || lower_name.contains("nigi")
                        || lower_name.contains("nigu")
                        || lower_name.contains("niga")
                        || lower_name.contains("nigo")
                        || lower_name.contains("niqq")
                        
                        || lower_name.contains("chink")
        
                        || lower_name.contains("wank")
                        || lower_name.contains("shit")
                        || lower_name.contains("cunt")
                        || lower_name.contains("bitch")
                        || lower_name.contains("booty")
                        || lower_name.contains("hooker")
                        || lower_name.contains("milf")
                        || lower_name.contains("rapist")
                        || lower_name.contains("balls")
                        || lower_name.contains("sex")
        
                        || lower_name.contains("cocaine")
                        || lower_name.contains("heroine")
                        || lower_name.contains("weed")
                        || lower_name.contains("weed")
                        || lower_name.contains("drug")				
        
                        || lower_name.contains("card")
        
                        || lower_name.contains("fisting")
                        || lower_name.contains("jerk")
        
                        || lower_name.contains("p3do")
                        || lower_name.contains("pedo")
                        || lower_name.contains("cplove")
                        || lower_name.contains("perv")
                                        
                        || lower_name.contains("gangbang")
                        || lower_name.contains("porn")
                        || lower_name.contains("dick")
                        || lower_name.contains("penis")
                        || lower_name.contains("puzzy")
                        || lower_name.contains("pussy")
                        || lower_name.contains("boceta")  // boceta is pussy in some lang
                        || lower_name.contains("anal")
                        || lower_name.contains("cum")
        
                        || lower_name.contains("market")
                        || lower_name.contains("sell")
                        || lower_name.contains("fraud")
                        //|| lower_name.contains("xoxo")
                        //|| lower_name.contains("m1l1tant")
        
                        || lower_name.contains("DN37R34P3R ")
                    ...
                    {
                        let msg = format!("do not use names that are on the blacklist names '{}'  bot ~Dantca",lower_name);
...
fn render_textbox(f: &mut Frame<CrosstermBackend<io::Stdout>>, app: &mut App, r: Rect) {
    ...
    let input = Paragraph::new(input_str)
        ...
        .block(Block::default().borders(Borders::ALL).title("Kirim"));
    ...
    let messages_list_items: Vec<ListItem> = messages
        ...
        .filter_map(|m| {
            ...
            let style = match (m.deleted, m.hide) {
                (true, _) => Style::default().bg(tuiColor::Rgb(139, 0, 0)),
...
fn render_users(f: &mut Frame<CrosstermBackend<io::Stdout>>, r: Rect, users: &Arc<Mutex<Users>>) {
    ...
    users_types.push((&users.admin, "-- GOD --"));
    users_types.push((&users.staff, "-- My Dream --"));
    users_types.push((&users.members, "-- Anggota --"));
    users_types.push((&users.guests, "-- Beban anjit --"));
